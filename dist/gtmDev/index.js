(function () {
	class $2668c19cb086d654$export$7e82c3fd8fbe1a4e {
		 ...
		 */ addVimeoListeners(el) {
			...
			// const cache = {};
			video.getVideoTitle().then((title1) => {
				['play', 'pause', 'complete'].forEach((key) => {
					if (this.config.events[`${key}`])
						video.on(vimeoVideoEvents[`${key}`], () => {
							this.updateDataLayer(key, title1);
						});
				});
			});
			const percentages =
				(_this_config__track = this.config._track) === null ||
				_this_config__track === void 0
					? void 0
					: _this_config__track.percentages;
			if (percentages)
				video.on('timeupdate', ({ percent: percent }) => {
					var key;
